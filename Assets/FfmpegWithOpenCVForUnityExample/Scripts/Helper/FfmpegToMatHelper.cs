...
namespace FfmpegWithOpenCVForUnity.UnityUtils.Helper
{
    ...
    public class FfmpegToMatHelper : MonoBehaviour
    {
        ...
        public virtual Mat GetMat()
        {
            ...
            if (baseColorFormat == outputColorFormat)
            {
                OpenCVMatUtils.Texture2DToMat(readCommand.VideoTextures[0].VideoTexture as Texture2D, frameMat, false);
            }
            else
            {
                OpenCVMatUtils.Texture2DToMat(readCommand.VideoTextures[0].VideoTexture as Texture2D, baseMat, false);
